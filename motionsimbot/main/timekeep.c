...
static void time_sync_cb(struct timeval *tv) { s_synced = true; }
...
time_t timekeep_now(void) { time_t now; time(&now); return now; }
void timekeep_format(char *buf, size_t len) {
    time_t now = timekeep_now(); struct tm ti; localtime_r(&now, &ti);
    strftime(buf, len, "%Y-%m-%d %H:%M:%S", &ti);
}
void timekeep_set_time(time_t epoch) {
    struct timeval tv = { .tv_sec = epoch, .tv_usec = 0 };
    settimeofday(&tv, NULL); s_synced = true;
}
void timekeep_schedule(const char *action, time_t execute_at) { timekeep_schedule_ms(action, execute_at, 0); }
...
static void scheduler_task(void *pvParameters) {
    while (1) {
        ...
        time_t now_s = tv.tv_sec; int32_t now_ms = tv.tv_usec / 1000;
        xSemaphoreTake(s_sched_mutex, portMAX_DELAY);
        for (int i = 0; i < MAX_SCHEDULED_ACTIONS; i++) {
            if (!s_schedule[i].active) continue;
            bool due = (now_s > s_schedule[i].execute_at) || (now_s == s_schedule[i].execute_at && now_ms >= s_schedule[i].execute_at_ms);
            if (due) {
...
void timekeep_start_scheduler(void) { xTaskCreate(scheduler_task, "sched", 3072, NULL, 3, NULL); }
